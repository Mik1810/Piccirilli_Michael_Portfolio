...
- Eventually, i.e when the file that are going to be typechecked or linted are changed, run `npm typecheck` and `npm lint` to ensure there are no type errors or linting issues before deploying.
- Eventually, i.e. when the files that are going to be tested are changed, run `npm test test:api` to ensure all tests are passing before deploying.
